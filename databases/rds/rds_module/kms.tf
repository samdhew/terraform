...
resource "aws_kms_key" "rds_kms_key" {
  description              = local.rds_kms_name
  key_usage                = "ENCRYPT_DECRYPT"
  customer_master_key_spec = "SYMMETRIC_DEFAULT"
  is_enabled               = true
  enable_key_rotation      = true
  multi_region             = var.rds_kms_multiregion
  tags                     = merge(local.tags, { Name : local.rds_kms_name })
}

resource "aws_kms_alias" "rds_kms_alias" {
  target_key_id = aws_kms_key.rds_kms_key.key_id
  name          = "alias/${local.rds_kms_name}"
}

resource "aws_kms_key_policy" "rds_kms_keypolicy" {
  key_id = aws_kms_key.rds_kms_key.key_id
  policy = jsonencode({
    "Id" = "rds_kms_keypolicy",
    "Statement" = [
      {
        "Action" = "kms:*",
        "Effect" = "Allow",
