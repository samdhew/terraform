...
resource "aws_kms_key" "dynamodb_kms_key" {
  description              = local.dynamodb_kms_name
  key_usage                = "ENCRYPT_DECRYPT"
  customer_master_key_spec = "SYMMETRIC_DEFAULT"
  is_enabled               = true
  enable_key_rotation      = true
  multi_region             = var.dynamodb_kms_multiregion
  tags                     = merge(local.tags, { Name : local.dynamodb_kms_name })
}

resource "aws_kms_alias" "dynamodb_kms_alias" {
  target_key_id = aws_kms_key.dynamodb_kms_key.key_id
  name          = "alias/${local.dynamodb_kms_name}"
}

resource "aws_kms_key_policy" "dynamodb_kms_keypolicy" {
  key_id = aws_kms_key.dynamodb_kms_key.key_id
  policy = jsonencode({
    "Id" = "dynamodb_kms_keypolicy",
    "Statement" = [
      {
        "Action" = "kms:*",
        "Effect" = "Allow",
