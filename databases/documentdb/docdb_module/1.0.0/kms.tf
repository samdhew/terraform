...
resource "aws_kms_key" "docdb_kms_key" {
  description              = local.docdb_kms_name
  key_usage                = "ENCRYPT_DECRYPT"
  customer_master_key_spec = "SYMMETRIC_DEFAULT"
  is_enabled               = true
  enable_key_rotation      = true
  multi_region             = var.docdb_kms_multiregion
  tags                     = merge(local.tags, { Name : local.docdb_kms_name })
}

resource "aws_kms_alias" "docdb_kms_alias" {
  target_key_id = aws_kms_key.docdb_kms_key.key_id
  name          = "alias/${local.docdb_kms_name}"
}

resource "aws_kms_key_policy" "docdb_kms_keypolicy" {
  key_id = aws_kms_key.docdb_kms_key.key_id
  policy = jsonencode({
    "Id" = "docdb_kms_keypolicy",
    "Statement" = [
      {
        "Action" = "kms:*",
        "Effect" = "Allow",
